...
| # | Date | What changed |
|---|---|---|
| 24 | 2026-04-06 | **AI Workout Review** — after finishing a workout, "AI Workout Review" card with ChatGPT/Claude/Copilot selector; generates a formatted prompt with workout stats, per-exercise set tables, previous session comparison, and asks the AI to rate and analyse; copies prompt to clipboard and opens the chosen app in a new tab; AI app preference persisted in localStorage |
| 23 | 2026-04-06 | **Accent color picker** — 8 theme presets (Lime, Blue, Cyan, Pink, Purple, Orange, Red, Gold) in Settings > Appearance; `ThemeProvider` applies saved accent via CSS custom properties on mount; `.text-gold`, `.bg-gold`, `.glow-gold`, `card-luxury`, muscle recovery bars, and charts all respond dynamically; persisted in localStorage |
